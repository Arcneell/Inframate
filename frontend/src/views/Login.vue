    ...
    <div class="relative z-10 w-full max-w-md px-6">
        <div class="bg-slate-900/80 backdrop-blur-xl border border-white/10 p-8 rounded-2xl shadow-2xl">
            <div class="text-center mb-8">
                ...
                <h1 class="text-3xl font-bold text-white tracking-tight">{{ t('loginTitle').value }}</h1>
                <p class="text-blue-200/70 mt-2 text-sm">{{ t('loginSubtitle').value }}</p>
            </div>

            <form @submit.prevent="handleLogin" class="space-y-6">
                <div class="space-y-1">
                    <label class="text-xs font-bold text-blue-100 uppercase tracking-wider ml-1">{{ t('username').value }} <span class="text-red-500">*</span></label>
                ...
                <div class="space-y-1">
                    <label class="text-xs font-bold text-blue-100 uppercase tracking-wider ml-1">{{ t('password').value }} <span class="text-red-500">*</span></label>
                    <div class="relative group">
                ...
                >
                    ...
                    <span>{{ t('signIn').value }}</span>
                </button>
            </form>

            <div class="mt-8 text-center">
                <p class="text-[10px] text-slate-600 uppercase tracking-widest">{{ t('authOnly').value }}</p>
            </div>
        </div>
    </div>
  </div>
...
const handleLogin = async () => {
    if (!username.value || !password.value) {
        toast.add({ severity: 'warn', summary: t('accessDenied').value, detail: 'Please fill all fields', life: 3000 });
        return;
    }
    loading.value = true;
    ...
    try {
        ...
        toast.add({ severity: 'success', summary: t('accessGranted').value, detail: t('welcomeBack').value, life: 2000 });
        ...
        toast.add({ severity: 'error', summary: t('accessDenied').value, detail: msg, life: 3000 });
    } finally {
        loading.value = false;
    }
};
